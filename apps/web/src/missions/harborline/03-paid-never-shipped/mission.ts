...
export const paidNeverShipped: Mission = {
  ...
  explanation: {
    summary:
      '`INNER JOIN` answers "what matches?" — a `LEFT JOIN` also answers "what doesn\'t?". It keeps every row from the left table and, where no shipment matches, fills the shipment columns with `NULL`; the filter `s.id IS NULL` therefore isolates exactly the orders no shipment ever referenced. `IS NULL` is required because `NULL` is not a value that `=` can match — it is the absence of one. The status filter does the other half of the work: pending and cancelled orders lack shipments legitimately, so `WHERE o.status = \'confirmed\'` keeps only the absences that should not exist.',
    concepts: [
      "LEFT JOIN",
      "IS NULL",
      "NULL semantics",
