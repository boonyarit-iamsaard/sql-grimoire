...
function renderValue(value: SqlValue) {
  if (value === null) {
    return <span className="text-ctp-overlay0 italic">NULL</span>;
  }
  if (value instanceof Uint8Array) {
    return <span className="text-ctp-overlay0 italic">[blob]</span>;
  }
  return String(value);
}
  ...
  return (
    <div className="rounded-xl border-2 border-ctp-surface1 bg-ctp-base px-3.5 py-3 shadow-paper">
      <h2 className="mb-2 text-base">
        Query Result{" "}
        <span className="ml-2 font-mono text-ctp-overlay1 text-xs">
      ...
      {result.rows.length === 0 ? (
        ...
        <div className="max-h-[320px] overflow-x-auto overflow-y-auto rounded-lg">
          <table className="w-full border-collapse font-mono text-mono">
            <thead>
              <tr>
                {result.columns.map((col) => (
                  <th
                    className="sticky top-0 whitespace-nowrap bg-ctp-surface0 px-3 py-[7px] text-left text-ctp-yellow"
                    key={col}
                  >
                    {col}
                  </th>
